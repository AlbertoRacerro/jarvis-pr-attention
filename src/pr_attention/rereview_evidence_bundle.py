...
def _require_source_failed_bundle(source: dict[str, Any], repo: str, number: int) -> tuple[str, str, str | None]:
    verification = verify_evidence_bundle(source)
    if not verification.valid:
        raise ValueError("source failed bundle is invalid: " + "; ".join(verification.reasons))
    if source.get("repository") != repo or _strict_pr(source.get("pr_number")) != number:
        raise ValueError("source failed bundle repository/pr_number does not match current snapshot")
    if source.get("phase") != "INTEGRATION_EVALUATED":
        raise ValueError("source failed bundle must be INTEGRATION_EVALUATED")
    if source.get("semantic_review_status") != "VALID_FAIL" or source.get("integration_gate_status") != "REPAIR":
        raise ValueError("source bundle is not a reusable failed semantic-review checkpoint")
    source_digest = source.get("bundle_sha256")
    previous_head = source.get("head_sha")
    accepted_head = source.get("accepted_head_sha")
    if not isinstance(source_digest, str) or not _DIGEST.fullmatch(source_digest):
        raise ValueError("source failed bundle digest is invalid")
    if not _valid_sha(previous_head):
        raise ValueError("source failed bundle head_sha is invalid")
    if accepted_head is not None and not _valid_sha(accepted_head):
        raise ValueError("source failed bundle accepted_head_sha is invalid")
    return source_digest, previous_head, accepted_head
...
def _require_packet(
    ...
    *,
    ...
    # The template builder performs the strict packet-shape checks used by the re-review validator.
    ...
    if not isinstance(supplied, str) or not _DIGEST.fullmatch(supplied) or supplied != expected:
...
def build_rereview_evidence_bundle(
    ...
    *,
    ...
    source_digest, previous_head, accepted_head = _require_source_failed_bundle(source_failed_bundle, repo, number)
    ...
    if validation is not None:
        ...
        if validation.get("previous_reviewed_head_sha") != previous_head or validation.get("head_sha") != head:
...
def verify_rereview_evidence_bundle(bundle: dict[str, Any]) -> RereviewBundleVerification:
    ...
    try:
        ...
        if not isinstance(supplied, str) or not _DIGEST.fullmatch(supplied) or supplied != rereview_bundle_sha256(bundle):
        ...
        for key in (
            ...
            "previous_reviewed_head_sha", "head_sha", "final_head_sha", "phase",
            "incremental_eligible", "packet_coverage", "packet_complete", "semantic_review_status",
            "integration_gate_status", "merge_ready", "next_action_class", "source_bundle_sha256",
            "component_digests", "trust", "evidence", "bundle_sha256",
        ):
